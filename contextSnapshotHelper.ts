...
// Update AgentContractSchema interface to match Zod schema
type AgentContractSchemaType = z.infer<typeof AgentContractSchema>;
interface AgentContracts {
  agentFiles: string[];
  schemas: AgentContractSchemaType[];
}
...
async function extractAgentContracts(): Promise<AgentContracts> {
    ...
    try {
      ...
      if (schema && typeof schema === 'object' && 'title' in schema && 'properties' in schema) {
        const validated = AgentContractSchema.safeParse({
          ...
          input: (schema as { properties: { input?: unknown } }).properties.input,
          output: (schema as { properties: { output?: unknown } }).properties.output
        });
  ...
  const schemas = schemasRaw.filter((s): s is AgentContractSchemaType => !!s && typeof s.file === 'string' && typeof s.title === 'string');
  return {
    agentFiles,
    schemas
  };
    ...
    try {
      ...
      lines.forEach((line, idx) => {
        if (/TODO|FIXME|PLANNED/.test(line)) {
          ...
          if (validated.success) todos.push(validated.data);
        }
      });
