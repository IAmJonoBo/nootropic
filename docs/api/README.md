...
> **Note:** The canonical sources for all documentation, backlog, and LLM/agent introspection are [`docs/docManifest.json`](_media/docManifest.json) and [`.nootropic-cache/describe-registry.json`](./nootropic-cache/describe-registry.json). All code/doc sync, describe registry, and CI enforcement follow [2025 TypeScript monorepo best practices](https://medium.com/@nikhithsomasani/best-practices-for-using-typescript-in-2025-a-guide-for-experienced-developers-4fca1cfdf052). See CONTRIBUTING.md for details.

**Navigation:**
- **Capabilities:** [docs/capabilities/](./docs/capabilities/) (implemented, registry-compliant only)
- **Epics:** [docs/epics/](./docs/epics/) (all planned, in-progress, and completed epics; auto-generated from backlog)
- **Stories:** [docs/stories/](./docs/stories/) (all actionable stories/subtasks; auto-generated from backlog)
- **Doc Manifest:** [docs/docManifest.json](_media/docManifest.json) (machine-readable, always up to date)
- **Backlog:** [agentBacklog.json](_media/agentBacklog.json) (annotated, canonical)

**Changelog:**
- All epics and stories are now discoverable and navigable in [docs/epics/](./docs/epics/) and [docs/stories/](./docs/stories/), with slugs, doc paths, and lastSynced fields.
- The doc manifest ([docs/docManifest.json](_media/docManifest.json)) now includes all capabilities, epics, and stories for LLM/agent workflows and automation.
- All planned/unimplemented docs have been moved out of [docs/capabilities/](./docs/capabilities/) to ensure only implemented, registry-compliant docs remain for agent/LLM discovery.
- All automation scripts and navigation are updated for the new structure. See [generateDocManifest.ts](_media/generateDocManifest.ts) and [generateBacklogSummary.ts](_media/generateBacklogSummary.ts).
- See CONTRIBUTING.md for docs-as-code and automation best practices.
...
- **See the canonical, always-up-to-date feature table and describe registry reference:**
  - [docs/capabilities.md](_media/capabilities.md) (auto-generated, canonical)
  - [Machine-readable registry](_media/describe-registry.json)
...
> **The canonical, always-up-to-date feature table is now in [docs/capabilities.md](_media/capabilities.md).**
...
- See [docs/llm-integration.md](_media/llm-integration.md) for a deep dive on LLM/agent integration, tool use, and best practices.
...
See the [llm-integration.md](_media/llm-integration.md) for more details on agent introspection and tool registry integration.
...
- See [docs/memoryLaneGuide.md](_media/memoryLaneGuide.md) for full details and API examples.
...
- **See full details and sample configs in [`docs/orchestration.md`](_media/orchestration.md#pre-commit--husky-hooks-for-json-schema-validation).**
...
All contributors should read the [CONTRIBUTING.md](_media/CONTRIBUTING.md) file before starting. It covers:
...
**For linting migration and troubleshooting, see [docs/orchestration.md](_media/orchestration.md#eslint-9-migration--linting-best-practices-2024).**

**See [CONTRIBUTING.md](_media/CONTRIBUTING.md) for full onboarding, TSDoc/TypeDoc/linting, and monorepo TypeScript config details. All utilities are now grouped by domain for maintainability and discoverability.**
...
See [docs/orchestration.md](_media/orchestration.md#typescript-linting--type-safety-best-practices-20242025) for actionable steps and references.

+**For ESLint 9+ migration and troubleshooting, see [docs/orchestration.md](_media/orchestration.md#eslint-9-migration--linting-best-practices-2024).**
...
> **Note:** All plugin lifecycle, OTel, health, and error reporting now follow the latest 2025 OTel/GenAI SIG best practices. See `pluginRegistry.ts`, `plugins/examplePlugin.ts`, and the canonical entries in [agentBacklog.json](_media/agentBacklog.json#note-latest-best-practices) and [docs/ROADMAP.md](_media/ROADMAP.md#plugin-health-checks--error-reporting) for up-to-date guidance.
...
- **Onboarding Checklist:**
  - See the new onboarding checklist in [CONTRIBUTING.md](_media/CONTRIBUTING.md#onboarding-checklist-doccode-sync--canonical-sources-2025-best-practices) for step-by-step instructions and troubleshooting. All new modules must export a `describe()` and will be automatically included in the registry.
- **Troubleshooting:**
...
- **Best Practices:**
  ...
  - See [CONTRIBUTING.md](_media/CONTRIBUTING.md) for onboarding and workflow details.
- **References:**
...
- **Security & Secrets Management:**
  - Vault/ArgoCD/Kaniko are integrated for secure CI/CD and secrets management. See [scripts/vaultIntegrationGuide.md](_media/vaultIntegrationGuide.md), [scripts/kanikoBuildExample.md](_media/kanikoBuildExample.md), and [scripts/argoCdIntegrationGuide.md](_media/argoCdIntegrationGuide.md). Discoverable via the describe registry and /capabilities endpoint.
...
- **Troubleshooting:**
  - If issues are missing or labels are out of sync, run the script locally with `--dry-run` to preview changes, or check the logs in the [backlog-sync.yml](_media/backlog-sync.yml) workflow.
...
- **Cross-tool/Org-wide SAST Feedback:**
  ...
  - **KMS/Vault integration:** Envelope encryption, key rotation, and audit logging for all remote/org-wide SAST feedback ([docs](_media/quality.md#kmsvault-integration-for-sast-feedback-encryption-2025)). AWS KMS implemented; GCP, Azure, Vault next. See docs/quality.md for usage and compliance.
  - **OIDC/JWT Security:** All remote APIs require OIDC/JWT authentication and validation ([docs](_media/quality.md#security-oidcjwt-for-remote-sast-feedback-apis-2025))
...
nootropic now supports declarative, composable agent workflows defined in YAML or JSON. Workflows are validated using a canonical JSON Schema and Zod schema (see `workflowSchema.ts`), and can be loaded, composed, and executed using the workflow composer (`composeWorkflows.ts`).
...
- **How it works:**
  ...
  - Steps can depend on each other, and are executed in dependency order (topological sort)
  - All workflows, agents, and tools are self-describing and discoverable
  - **Advanced step types:**
    - `task`: Standard agent/tool invocation
    - `approval`: Human-in-the-loop approval (emits rationale/approval events, can use approvalHandler)
    - `parallel`: Runs child steps concurrently (see `metadata.children`)
    - `loop`: Stub for repeated execution (extension point for LLM/AI-driven iteration)
  - **Hooks and events:**
    - `preStep`/`postStep` hooks for policy/mutation
    - `rationaleHandler` for explainability events
    - `approvalHandler` for custom approval logic (HITL/LLM)
- **References:**
  - [`workflowSchema.ts`](_media/workflowSchema.ts) (schema)
  - [`composeWorkflows.ts`](_media/composeWorkflows.ts) (loader/runner)
  - [`orchestration/examples/simple-workflow.yaml`](_media/simple-workflow.yaml) (example)
  - [`docs/orchestration.md`](_media/orchestration.md) (architecture, best practices)

**Example: Advanced Workflow Execution**

```ts
import { loadWorkflow, executeWorkflow } from './composeWorkflows';
const workflow = loadWorkflow('orchestration/examples/simple-workflow.yaml');
await executeWorkflow(workflow, {
  preStep: async (step) => {
    // Policy check or mutation
    console.log('About to execute', step.id);
  },
  postStep: async (step, result) => {
    // Audit/logging
    console.log('Completed', step.id, result);
  },
  approvalHandler: async (step) => {
    // Custom approval logic (HITL/LLM)
    return { approved: true, rationale: 'Approved by test harness' };
  },
  rationaleHandler: async (step, rationale) => {
    // Emit rationale/explainability event
    console.log('Rationale:', rationale);
  }
});
```

This approach is aligned with 2025 best practices for agentic systems, supporting event-driven choreography, composability, robust validation, and LLM/AI explainability. See the backlog and docs for implementation details.
...
- Doc/code sync and describe registry are now fully in sync. All implemented capabilities are discoverable and LLM/AI-friendly. Planned/obsolete stubs have been pruned from the doc manifest.
...
- **Vault, Kaniko, and ArgoCD Managers:** All three are now scaffolded, registry-compliant, and ready for implementation. See [scripts/vaultIntegrationGuide.md](_media/vaultIntegrationGuide.md), [scripts/kanikoBuildExample.md](_media/kanikoBuildExample.md), and [scripts/argoCdIntegrationGuide.md](_media/argoCdIntegrationGuide.md). Discoverable via the describe registry and /capabilities endpoint.
...
- **CLI/API:** See [docs/memoryLaneGuide.md](_media/memoryLaneGuide.md#checkpoint-based-rollback--restoration-local-jsonl) for usage examples and advanced options.
...
## 🚀 Automation, CLI, and Onboarding Docs
...
### Automation Scripts (LLM/AI-Usable)
...
> **LLM/AI Usage Hints:** All scripts support `--json` for machine-readable output. See the CLI usage guide for more.

## Schema Validation for Plugins and Capabilities

All plugins and registry capabilities are now validated at runtime using canonical [Zod](https://zod.dev/) schemas. This ensures type safety, LLM/AI-friendliness, and robust automation. See:

- [Schema Validation Onboarding](docs/capabilities/schema%20validation%20onboarding%20(planned).md)
- [Plugin Architecture](docs/capabilities/plugin%20architecture%20(planned).md)
- [Agent and Plugin Discovery](docs/capabilities/agent%20discovery%20(planned).md)

Plugins and capabilities must export a `describe()` function and conform to the canonical schema. Invalid modules are rejected at runtime and logged for debugging.

## 📝 Changelog

- 2025-05-17: Doc/code sync and describe registry: fully in sync, all implemented capabilities discoverable and LLM/AI-friendly. Planned/obsolete stubs pruned from doc manifest.
- 2025-05-17: Lifecycle compliance: all modules export async hooks, and compliance test is robust to transpilation
- 2025-05-17: LLM adapter integration: rerankers now call adapter methods, so tests are meaningful and pass
- 2025-05-17: Kafka integration test requires a running broker or a mock for CI (backlog)
- 2025-05-17: contextSnapshotHelper test: one test times out, needs timeout/mocking review (backlog)
- Deduplicated and type-checked `secretsManager.ts`; all type errors resolved. Codebase is now robust, DRY, and LLM/AI-friendly.
...
- See [agentBacklog.json](_media/agentBacklog.json) and [docs/quality.md](_media/quality.md) for details and current backlog state.
