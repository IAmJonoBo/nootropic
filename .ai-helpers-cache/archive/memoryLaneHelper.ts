...
class JsonlEventBus implements EventBusBackend {
  ...
  async publishToTopic(event: AgentEvent, topic?: string): Promise<void> {
    ...
    if (typeof topicName === 'string' && topicName && topicSubscribers[topicName]) {
...
const extendedEventBus = eventBus as ExtendedEventBus;
...
export async function publishToTopic(event: AgentEvent, topic?: string) { return extendedEventBus.publishToTopic?.(event, topic); }
export async function getEvents(filter?: Partial<AgentEvent>) { return extendedEventBus.getEvents?.(filter); }
export async function getEventsByTopic(topic: string) { return extendedEventBus.getEventsByTopic?.(topic); }
export function subscribe(callback: (event: AgentEvent) => void) { return extendedEventBus.subscribe?.(callback); }
...
export async function logEvent(level: LogLevel, message: string, details?: Record<string, unknown>, agentId?: string, correlationId?: string, topic?: string) { return extendedEventBus.logEvent?.(level, message, details, agentId ?? '', correlationId ?? '', topic ?? ''); }

// --- Basic event validation (align with event-schema.json) ---
// function validateEvent(event: AgentEvent): boolean {
//   return (
//     typeof event.type === 'string' &&
//     typeof event.timestamp === 'string' &&
//     (!event.agentId ?? typeof event.agentId === 'string')
//   );
// }
// Use canonical Zod-based validation
function validateEvent(event: AgentEvent): boolean {
