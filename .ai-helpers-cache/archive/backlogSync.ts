 ...
 * backlogSync.ts
 *
 * Synchronizes the backlog (agentBacklog.json) with the current state of the codebase, docs, and roadmap.
 *
 * Also escalates (creates/updates) GitHub Issues for TODOs with aiSuggestedPriority: 'top' or stale: true,
 * using special labels ('nootropic-hotspot', 'nootropic-stale').
 *
 * Usage:
 *   pnpm tsx scripts/backlogSync.ts [--help] [--json] [--dry-run]
 *
 * Flags:
 ...
 *
 * Output:
 *   Human-readable or JSON status message. Updates or validates the backlog as needed.
 *
 * Troubleshooting:
 *   - Ensure agentBacklog.json is present and writable.
 *   - Use --dry-run to preview changes without writing.
 *   - Use --json for machine-readable output in CI/CD.
 *   - For errors, check for invalid or missing backlog entries.
 *
 * Example:
 *   pnpm tsx scripts/backlogSync.ts --dry-run --json
 *
 * LLM/AI Usage Hints:
 *   - "Show usage for backlogSync script."
 *   - "List all scripts that sync backlog with GitHub Issues."
 *   - "How do I sync the backlog with GitHub Issues?"
 *   - "What does backlogSync do?"
 */
...
async function main() {
  ...
  try {
    ...
    for (const issue of issues) {
      if (issue.title) issueMap.set(issue.title, issue);
    }
    ...
    for (const todo of backlogTodos) {
      if (todo && todo.id && todo.id.startsWith('todo:') && (todo.aiSuggestedPriority === 'top' || todo.stale === true)) {
