...
function FeedbackSection({ pluginName }: { pluginName: string }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    setSuccess(true);
    const newFeedback: PluginFeedback = {
      rating,
      review,
      submittedAt: new Date().toISOString(),
    };
    setFeedbacks((prev: unknown) => [newFeedback, ...prev as PluginFeedback[]]);
    setRating(0);
    setReview('');
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      {open && (
        <form onSubmit={handleSubmit} style={{ marginTop: 8, background: '#f9f9f9', padding: 12, borderRadius: 8, border: '1px solid #eee' }}>
          <div>
            <label>Rating: </label>
            {[1,2,3,4,5].map((star) => (
          ...
          <button type="submit" style={{ marginTop: 8, padding: '2px 12px', borderRadius: 4, border: '1px solid #888', background: '#e0ffe0', cursor: 'pointer' }}>Submit</button>
        ...
        <>
          ...
          <ul>
            {filteredCapabilities.map((cap: Capability) => <li key={cap.id} style={{ marginBottom: 8 }}>
              ...
              <b>{cap.name}</b> {cap.description && <span style={{ color: '#888' }}>({cap.description})</span>}
              {cap.health && <span style={{ color: cap.health === 'healthy' ? 'green' : 'red', marginLeft: 8 }}>{cap.health}</span>}
              {!cap.health && <span style={{ color: '#888', marginLeft: 8 }}>unknown</span>}
            </li>)}
          </ul>
          <h2>Plugins</h2>
          <ul>
              ...
              return (
                <li key={plg.name} style={{ marginBottom: 12 }}>
                  ...
                  {typeof plg.meta?.description === 'string' && (
                    <span style={{ color: '#888', marginLeft: 8 }}>{plg.meta.description}</span>
                  )}
                  {health && <span style={{ color: health === 'healthy' ? 'green' : 'red', marginLeft: 8 }}>{health}</span>}
