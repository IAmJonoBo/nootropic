...
async function scanCacheDir(): Promise<CacheManifestEntry[]> {
  ...
  // @ts-expect-error TS(2304): Cannot find name 'manifest'.
  ...
  try {
    // @ts-expect-error TS(2304): Cannot find name 'archiveFiles'.
 ...
 */
// @ts-expect-error TS(6133): 'manifest' is declared but its value is never read... Remove this comment to see the full error message
...
async function cliEntrypoint(): Promise<void> {
  ...
  // @ts-expect-error TS(2304): Cannot find name 'positionals'.
  ...
  try {
    ...
    if (args['validate-manifest']) {
      ...
      // @ts-expect-error TS(6133): 'f' is declared but its value is never read.
