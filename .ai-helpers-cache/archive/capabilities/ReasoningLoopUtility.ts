 ...
 */
// @ts-expect-error TS(2420): Class 'ReasoningLoopUtility' incorrectly implement... Remove this comment to see the full error message
export class ReasoningLoopUtility implements Capability {
   ...
   */
  // @ts-expect-error TS(7010): 'runLoop', which lacks return-type annotation, imp... Remove this comment to see the full error message
  async runLoop(
    ...
    options?: {
      ...
      // @ts-expect-error TS(2304): Cannot find name 'structuredReasoning'.
      structuredReasoning?: boolean;
      // @ts-expect-error TS(2304): Cannot find name 'uncertaintyFn'.
      ...
      // @ts-expect-error TS(2304): Cannot find name 'llmRepair'.
      llmRepair?: (step: string, context: string[]) => Promise<string>;
      // @ts-expect-error TS(2304): Cannot find name 'maxBacktrackSteps'.
      maxBacktrackSteps?: number;
    }
  // @ts-expect-error TS(2365): Operator '<' cannot be applied to types 'PromiseCo... Remove this comment to see the full error message
  ): Promise<{ result: string; log: string[] }> {
    // @ts-expect-error TS(2552): Cannot find name 'log'. Did you mean 'Long'?
    const log: string[] = [];
    // @ts-expect-error TS(2304): Cannot find name 'maxSteps'.
    const maxSteps = options?.maxSteps ?? 3;
    // @ts-expect-error TS(2304): Cannot find name 'allowBacktrack'.
    const allowBacktrack = options?.allowBacktrack ?? true;
    // @ts-expect-error TS(2304): Cannot find name 'uncertaintyThreshold'.
    ...
    // @ts-expect-error TS(6133): 'structuredReasoning' is declared but its value is... Remove this comment to see the full error message
    if (structuredReasoning) {
      // @ts-expect-error TS(2339): Property 'generateStructuredPlan' does not exist o... Remove this comment to see the full error message
    ...
    for (let i = 0; i < maxSteps; i++) {
      ...
      if (confidence < uncertaintyThreshold) {
        // @ts-expect-error TS(2304): Cannot find name 'Uncertainty'.
  ...
  async generateStructuredPlan(input: string, llmExplain?: (step: string, context: string[]) => Promise<string>): Promise<string> {
    if (llmExplain) {
      // @ts-expect-error TS(2304): Cannot find name 'Decompose'.
      return llmExplain(`Decompose the following problem into sequence/branch/loop steps: ${input}`, []);
    }
    // Fallback: simple template
    // @ts-expect-error TS(2304): Cannot find name 'Analyze'.
  ...
  async explainStep(step: string): Promise<string> {
    ...
    // @ts-expect-error TS(2304): Cannot find name 'Explanation'.
