    ...
    if (req.method === 'GET' && req.url === '/context') {
      ...
      req.on('end', async () => {
        try {
          ...
          if (typeof source === 'string') opts.source = source;
          if (typeof capability === 'string') opts.capability = capability;
      ...
      req.on('end', async () => {
        try {
          ...
          const opts: { maxAgeDays?: number; maxSizeBytes?: number } = {};
          if (typeof options.maxAgeDays === 'number') opts.maxAgeDays = options.maxAgeDays;
          if (typeof options.maxSizeMB === 'number') opts.maxSizeBytes = options.maxSizeMB * 1024 * 1024;
      ...
      req.on('end', async () => {
        try {
          const { dryRun } = body ? JSON.parse(body) : {};
          const manifest = await contextManager.listCacheFiles();
          const toCompress = manifest.filter((entry: unknown) => {
      ...
      req.on('end', async () => {
        try {
          const { fileName, dryRun } = body ? JSON.parse(body) : {};
          if (!fileName) throw new Error('fileName is required');
          const manifest = await contextManager.listCacheFiles();
          const entry = manifest.find((f: unknown) => typeof f === 'object' && f !== null && 'fileName' in f && (f as Record<string, unknown>)['fileName'] === fileName);
          if (!entry || !(entry as Record<string, unknown>)['archive']) throw new Error('File not found in archive');
      ...
      req.on('end', async () => {
        try {
          const options = body ? JSON.parse(body) : {};
          await contextManager.pruneContext(options);
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ success: true, action: 'prune' }));
        } catch (e) {
          res.writeHead(500, { 'Content-Type': 'application/json' });
      ...
      req.on('end', async () => {
        try {
          const { tier } = body ? JSON.parse(body) : {};
      ...
      req.on('end', async () => {
        try {
          const { fileName, targetTier } = JSON.parse(body);
      ...
      req.on('end', async () => {
        try {
          const { contextArr, agentConfig, modelTokenLimit } = body ? JSON.parse(body) : {};
      ...
      if (req.method === 'GET' && matchPrompts) {
        ...
        try {
          ...
          if (desc && typeof desc === 'object' && 'promptTemplates' in desc) {
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify((desc as { promptTemplates?: unknown[] }).promptTemplates ?? []));
          } else {
