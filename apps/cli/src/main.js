...
program
  ...
  .action(async (options) => {
    try {
      ...
      const result = await coderService.generateCode(
        options.prompt,
        options.context,
      );
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const review = await criticService.reviewCode(
        options.code,
        options.standards,
      );
...
program
  ...
  .option(
    "-t, --type <type>",
    "Feedback type (success|failure|improvement|bug)",
  )
