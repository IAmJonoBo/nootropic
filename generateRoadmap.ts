...
function renderFeature(item: unknown) {
  ...
  let out = `### ${mdEscape((obj['feature'] as string ?? obj['title'] as string) || 'Untitled')}`;
  if (obj['priority']) out += ` \`[${obj['priority']}]\``;
  if (obj['rationale'] ?? obj['description']) out += `\n${obj['rationale'] ?? obj['description']}`;
  if (obj['actionableSteps'] && Array.isArray(obj['actionableSteps']) && (obj['actionableSteps'] as unknown[]).length) {
    out += '\n**Actionable Steps:**';
    for (const step of obj['actionableSteps'] as unknown[]) out += `\n- ${mdEscape(String(step))}`;
  }
  if (obj['references'] && Array.isArray(obj['references']) && (obj['references'] as unknown[]).length) {
    out += '\n**References:**';
    for (const ref of obj['references'] as unknown[]) out += `\n  - ${String(ref)}`;
  }
  if (obj['researchLinks'] && Array.isArray(obj['researchLinks']) && (obj['researchLinks'] as unknown[]).length) {
    out += '\n**Research Links:**';
    for (const ref of obj['researchLinks'] as unknown[]) out += `\n  - ${String(ref)}`;
  }
