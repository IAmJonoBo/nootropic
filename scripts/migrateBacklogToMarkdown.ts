...
function renderMarkdown(item: any, type: 'epic' | 'story', parentId?: string): string {
  const lines = [
    `# ${item.id} (${type})`,
    '',
    ...
    '---',
    '',
    item.subtasks && item.subtasks.length ? '## Subtasks\n' + item.subtasks.map((s: any) => `- [${s.id}](${s.docPath})`).join('\n') : '',
  ];
  return lines.filter(Boolean).join('\n');
}

function annotate(item: any, type: 'epic' | 'story'): any {
  ...
  if (item.subtasks && Array.isArray(item.subtasks)) {
    annotated.subtasks = item.subtasks.map((s: any) => annotate(s, 'story'));
  }
  return annotated;
}

function* walkStories(item: any): Generator<{ story: any; parentId?: string }> {
  if (item.subtasks && Array.isArray(item.subtasks)) {
...
async function main() {
  ...
  if (!fs.existsSync(backlogPath)) throw new Error('agentBacklog.json not found');
  ...
  const annotatedEpics = backlog.map((epic: any) => annotate(epic, 'epic'));
