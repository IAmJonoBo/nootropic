 ...
 */
export async function appendKeyAuditLog(entry) {
 ...
 */
export class KmsVaultSecretsManager {
...
export class VaultSecretsManager {
    ...
    async getSecret(name) {
        const span = trace.getTracer('secretsManager').startSpan('getSecret');
        try {
            if (!this.client || typeof this.client.read !== 'function')
                throw new Error('Vault client not initialized');
            const result = await this.client.read(`${this.mount}/data/${name}`);
            span.end();
    ...
    async setSecret(name, value) {
        ...
        try {
            if (!this.client || typeof this.client.write !== 'function')
                throw new Error('Vault client not initialized');
            await this.client.write(`${this.mount}/data/${name}`, { data: { value } });
            span.end();
        }
        catch (err) {
            span.recordException(err);
    ...
    async health() {
        try {
            if (!this.client || typeof this.client.health !== 'function')
                throw new Error('Vault client not initialized');
 ...
 */
export class KanikoBuildManager {
 ...
 */
export class ArgoCdDeploymentManager {
