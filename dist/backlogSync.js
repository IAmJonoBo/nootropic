    ...
    try {
        ...
        for (const item of actionable) {
            if (item && typeof item === 'object' && 'feature' in item)
                featureMap.set(item.feature, item);
        }
        ...
        for (const issue of issues) {
            if (issue && typeof issue === 'object' && 'title' in issue && issue.title)
        ...
        for (const [feature, item] of featureMap.entries()) {
            ...
            if (issueMap.has(title)) {
                ...
                const currentLabels = (issue.labels ?? []).map((l) => typeof l === 'string' ? l : l.name).filter((l) => typeof l === 'string');
                const missingLabels = labels.filter(l => !currentLabels.includes(l));
                const extraLabels = currentLabels.filter((l) => statusLabels.includes(l) && !labels.includes(l));
                if (issue.body !== body) {
                    actions.push({ type: 'update', number: issue.number, title, body });
                    if (!dryRun)
                        await octokit.issues.update({ owner, repo, issue_number: issue.number, body });
                }
                if (missingLabels.length > 0) {
                    actions.push({ type: 'addLabels', number: issue.number, labels: missingLabels });
                    if (!dryRun)
                        await octokit.issues.addLabels({ owner, repo, issue_number: issue.number, labels: missingLabels });
                }
                for (const l of extraLabels) {
                    actions.push({ type: 'removeLabel', number: issue.number, label: l });
                    if (!dryRun)
                        await octokit.issues.removeLabel({ owner, repo, issue_number: issue.number, name: l });
                }
        ...
        for (const todo of backlogTodos) {
            if (todo && typeof todo === 'object' && 'id' in todo && typeof todo.id === 'string' && todo.id.startsWith('todo:') && (todo.aiSuggestedPriority === 'top' || todo.stale === true)) {
                ...
                const labels = [label];
                if (todo.aiSuggestedPriority === 'top')
                    labels.push('nootropic-hotspot');
                if (todo.stale === true)
                    labels.push('nootropic-stale');
                if (issueMap.has(title)) {
                    const issue = issueMap.get(title);
                    if (issue.body !== body) {
                        actions.push({ type: 'update', number: issue.number, title, body });
                    ...
                    for (const l of missingLabels) {
                        ...
                        if (!dryRun)
                            await octokit.issues.addLabels({ owner, repo, issue_number: issue.number, labels: [l] });
                    }
                    issueMap.delete(title);
                }
                else {
        ...
        if (actions.length === 0) {
            ...
            for (const action of actions) {
                if (action.type === 'create')
                    console.log(`Would create: ${action.title}`);
                if (action.type === 'update')
                    console.log(`Would update: ${action.title}`);
                if (action.type === 'close')
                    console.log(`Would close: ${action.title}`);
            }
            if (!dryRun)
                console.log('Backlog sync: Actions completed.');
        }
