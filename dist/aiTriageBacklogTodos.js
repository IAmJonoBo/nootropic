 ...
 * LLM/AI-usage: Adds aiSuggestedTriage, aiSuggestedPriority, daysOld, and stale fields to each TODO. Outputs hotspots and stale items for automation and reporting.
 */
import fs from 'fs';
...
async function main() {
    ...
    try {
        ...
        for (const item of Array.isArray(backlogJson.backlog) ? backlogJson.backlog.filter((i) => typeof i === 'object' && i !== null && 'id' in i && typeof i.id === 'string' && (i.id).startsWith('todo:')) : []) {
        ...
        if (changed) {
            ...
            if (args['json'])
                printResult({ message: 'AI triage, priority, and staleness suggestions added to backlog.' }, true);
            else
                printResult('AI triage, priority, and staleness suggestions added to backlog.');
        }
