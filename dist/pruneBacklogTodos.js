...
async function main() {
    ...
    // Partition TODOs
    const [toArchive, toKeep] = backlogArr.reduce((acc, item) => {
        if (typeof item === 'object' && item !== null &&
            (('triage' in item && (item.triage === 'resolved' || item.triage === 'ignored')) ||
                ('status' in item && item.status === 'complete') ||
                ('stale' in item && item.stale === true))) {
            // Only archive if not already present
            if (!archiveArr.some((a) => typeof a === 'object' && a !== null &&
