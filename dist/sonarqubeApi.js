...
export async function getSonarQubeFindings(projectKey) {
    ...
    return tracer.startActiveSpan('sonarqube.getFindings', async (span) => {
        try {
            ...
            const findings = (data.issues ?? []).map((f) => {
...
export async function explainSonarQubeFinding(finding) {
...
export async function suggestSonarQubeFix(finding) {
    if (typeof finding !== 'object' || finding === null)
        return { suggestion: '', rationale: 'Invalid finding' };
    const f = finding;
    return {
...
export async function autoFixSonarQubeFinding(finding) {
