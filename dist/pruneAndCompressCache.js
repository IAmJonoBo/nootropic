    ...
    try {
        ...
        if (args['validate-manifest']) {
            ...
            try {
                ...
                manifest = manifest.filter((entry) => typeof entry.fileName === 'string');
            }
            catch {
                return printError('Manifest missing or unreadable. Run with --refresh-manifest.', args['json']);
            }
            const files = await scanCacheDir();
            const manifestFiles = new Set(manifest.map((f) => f.fileName));
            const actualFiles = new Set(files.map((f) => f.fileName));
