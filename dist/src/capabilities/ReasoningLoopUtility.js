...
export class ReasoningLoopUtility {
    ...
    async runLoop(input, options) {
        ...
        for (let i = 0; i < maxSteps; i++) {
            ...
            if (confidence < uncertaintyThreshold) {
                ...
                if (options?.emitEvent)
                    await options.emitEvent(explanationEvent);
            ...
            if (feedback) {
                ...
                if (action === 'backtrack' && allowBacktrack && i > 0 && backtrackCount < maxBacktrackSteps) {
                    ...
                    if (options?.emitEvent)
                        await options.emitEvent(repairEvent);
     ...
     */
    async generateStructuredPlan(input, llmExplain) {
        if (llmExplain) {
