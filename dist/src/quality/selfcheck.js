            ...
            try {
                ...
                await new Promise((resolve, reject) => {
                        ...
                        if (code === 0) {
                            try {
                                ...
                                if (!parsed.success) {
                                    ...
                                    span.setStatus({ code: 2, message: 'Invalid Semgrep output' });
                                    reject(new Error('Invalid Semgrep output'));
                                    return;
                                }
                                const findings = (result.results ?? []).map((f) => {
                                    if (typeof f !== 'object' || f === null)
                                        return null;
                                    const obj = f;
                                    let cwe, owasp;
                                    if (obj['extra'] && typeof obj['extra'] === 'object' && obj['extra'] !== null && 'metadata' in obj['extra']) {
                                        const meta = obj['extra']['metadata'];
                                        if (meta && typeof meta === 'object' && meta !== null) {
                                            cwe = 'cwe' in meta ? meta['cwe'] : undefined;
                                            owasp = 'owasp' in meta ? meta['owasp'] : undefined;
                                        }
                                    }
                                    return {
                                        id: `${String(obj['check_id'])}:${String(obj['path'])}:${(obj['start'] && typeof obj['start'] === 'object' && obj['start'] !== null && 'line' in obj['start']) ? String(obj['start']['line']) : ''}`,
                                        file: obj['path'],
                                        startLine: obj['start'] && typeof obj['start'] === 'object' && obj['start'] !== null && 'line' in obj['start'] ? obj['start']['line'] : undefined,
                                        endLine: obj['end'] && typeof obj['end'] === 'object' && obj['end'] !== null && 'line' in obj['end'] ? obj['end']['line'] : undefined,
                                        ruleId: obj['check_id'],
                                        message: obj['extra'] && typeof obj['extra'] === 'object' && obj['extra'] !== null && 'message' in obj['extra'] ? obj['extra']['message'] : '',
                                        severity: obj['extra'] && typeof obj['extra'] === 'object' && obj['extra'] !== null && 'severity' in obj['extra'] ? obj['extra']['severity'] : undefined,
                                        cwe,
                                        owasp,
                                        raw: f
                                    };
                                ...
                                span.setStatus({ code: 2, message: 'Parse error' });
                                reject(e);
                            }
                        }
                        else {
                            console.error(`[quality][semgrep] Semgrep scan failed with exit code ${code}.`);
                            span.setStatus({ code: 2, message: 'Semgrep scan failed' });
                            process.exitCode = code ?? 1;
                    ...
                    proc.on('error', (err) => {
                        ...
                        span.setStatus({ code: 2, message: 'Spawn error' });
                        process.exitCode = 1;
                        reject(err);
                    });
                });
 ...
 */
export async function getSemgrepFindings() {
 ...
 */
export async function explainSemgrepFinding(finding) {
 ...
 */
export async function suggestSemgrepFix(finding) {
 ...
 */
export async function autoFixSemgrepFinding(finding, codeContext) {
    ...
    if (patch && patch.trim().startsWith('// PATCH:'))
        syntaxValid = true;
