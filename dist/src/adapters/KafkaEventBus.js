...
import { context, SpanStatusCode, defaultTextMapSetter, defaultTextMapGetter } from '@opentelemetry/api';
...
export class KafkaEventBus {
        ...
        try {
            ...
            for (const entry of consumersEntries) {
                ...
                for (const partitionInfo of partitions) {
                    if (!partitionInfo)
                        continue;
                    if (typeof partitionInfo.partitionId !== 'number')
                        continue;
                    const partition = partitionInfo.partitionId;
                    const logEndOffset = Number((topicOffsets && Array.isArray(topicOffsets) ? topicOffsets.find((p) => p && p.partition === partition) : undefined)?.offset ?? 0);
                    const committedOffset = Number((groupOffsets && Array.isArray(groupOffsets) ? groupOffsets.find((p) => p && p.partition === partition) : undefined)?.offset ?? 0);
    ...
    constructor(brokers, clientId = 'nootropic') {
    ...
    async subscribeToTopic(topic, handler) {
        ...
        await consumer.run({
            eachMessage: async ({ topic, partition, message }) => {
                await tracer.startActiveSpan('KafkaEventBus.consumeEvent', async (span) => {
                    try {
                        if (message.headers) {
                        ...
                        try {
                            ...
                            const { DLQEventSchema } = await import('../schemas/AgentOrchestrationEngineSchema.js');
                            const dlqEvent = {
                                type: 'DLQ',
                            ...
                            await tracer.startActiveSpan('KafkaEventBus.produceDLQ', async (dlqSpan) => {
                                try {
    ...
    async publishEvent(event) {
        const tracer = getTracer('KafkaEventBus');
        await tracer.startActiveSpan('KafkaEventBus.publishEvent', async (span) => {
            try {
                const headers = {};
