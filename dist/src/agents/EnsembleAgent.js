...
// @ts-ignore
import { getEmbeddingBackend } from '../utils/embedding/embeddingClient.js';
export class MajorityVotingStrategy {
    name = 'majority';
...
export class SemanticVotingStrategy {
    name = 'semantic';
    backend;
    ...
    async vote(candidates, context) {
        if (!Array.isArray(candidates) || candidates.length === 0)
            return { winner: '', rationale: 'No candidates provided for semantic voting.', details: {} };
        if (!context || typeof context.query !== 'string')
            return { winner: candidates[0] ?? '', rationale: 'No query provided for semantic voting.', details: {} };
...
export class WeightedVotingStrategy {
    name = 'weighted';
    weights;
    constructor(weights) { this.weights = weights; }
    async vote(candidates) {
        if (!Array.isArray(candidates) || candidates.length === 0)
            return { winner: '', rationale: 'No candidates provided.', details: {} };
        if (!Array.isArray(this.weights) || this.weights.length !== candidates.length)
            return { winner: candidates[0] ?? '', rationale: 'Weights missing or mismatched, defaulting to first candidate.', details: this.weights };
...
export class SyntacticVotingStrategy {
    name = 'syntactic';
    async vote(candidates) {
    ...
    async runTask(task) {
        ...
        const emitEvent = async (event) => {
            ...
            switch (event.type) {
                case 'candidateGeneration': {
                    const { candidates, method } = event.payload || {};
                    safePayload = {
                ...
                case 'votingRationale': {
                    const { rationale, details, candidates, winner } = event.payload || {};
                    safePayload = {
                ...
                case 'metaLLMAdjudication': {
                    const { candidates, result } = event.payload || {};
                    safePayload = {
                        candidates: Array.isArray(candidates) ? candidates : [],
                ...
                case 'selfDebugging': {
                    const { candidates, status, log } = event.payload || {};
                    safePayload = {
                        candidates: Array.isArray(candidates) ? candidates : [],
        ...
        return {
            ...
            logs: [
                ...
                selfDebugLog,
                ...events.map(e => `[event] ${e.type}: ${JSON.stringify((e && typeof e === 'object' && 'payload' in e ? e.payload : {}) ?? {})}`)
            ].filter(Boolean)
        };
    }
    async health() {
