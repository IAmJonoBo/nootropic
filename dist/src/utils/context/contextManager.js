...
export class ContextManager {
     ...
     */
    async getCacheFile(fileName) {
        try {
            const manifest = await this.listCacheFiles();
            const entry = manifest.find((f) => isManifestEntry(f) && f.fileName === fileName);
            if (!entry)
    ...
    async getCacheHealthReport() {
        try {
            ...
            for (const entry of manifest) {
                if (typeof entry !== 'object' || entry === null || !('size' in entry) || !('lastModified' in entry))
                    continue;
     ...
     */
    async migrateFileTier(fileName, targetTier) {
     ...
     */
    async getContextSummaries(query, topK = 5) {
 ...
 */
export function generateTodoPatch(original, patched, file, line) {
    return `--- ${file}:${line}\n+++ ${file}:${line}\n@@ -${line},1 +${line},1 @@\n-${original}\n+${patched}`;
}
...
function getTierForFile(stats) {
    const ageDays = (Date.now() - stats.mtime.getTime()) / (1000 * 60 * 60 * 24);
    if (ageDays <= tierPolicy.hot.maxAgeDays && stats.size <= tierPolicy.hot.maxSizeMB * 1024 * 1024)
        return 'hot';
    if (ageDays <= tierPolicy.warm.maxAgeDays && stats.size <= tierPolicy.warm.maxSizeMB * 1024 * 1024)
        return 'warm';
    return 'cold';
}
async function migrateFileTier(fileName, targetTier) {
...
async function enforceTierPolicies() {
    ...
    for (const entry of manifest) {
        if (typeof entry !== 'object' || entry === null ||
            ...
            continue;
        const { fileName, tier, size, lastModified } = entry;
        if (tier === 'hot')
            continue;
        if (tier === 'warm') {
...
export async function extractWordsFromFile(filePath) {
    try {
        if (!filePath.endsWith('.json.js') && !filePath.endsWith('.jsonc')) {
