 ...
 */
export class SastMemoriesUtility extends BaseMemoryUtility {
    name = 'sastMemories';
    ...
    deduplicate(memories) {
        if (this.deduplicateFn)
            return this.deduplicateFn(memories);
        const map = new Map();
     ...
     */
    async getMemoriesForRule(ruleId) {
        const all = await this.listAllSastMemories();
     ...
     */
    async syncWithRemote(localMemories, remoteAdapter) {
        if (!remoteAdapter || typeof remoteAdapter !== 'object' ||
            typeof remoteAdapter.downloadMemories !== 'function' ||
            typeof remoteAdapter.mergeMemories !== 'function' ||
            typeof remoteAdapter.uploadMemories !== 'function') {
     ...
     */
    mergeWithRemote(local, remote) {
        const span = trace.getTracer('sastMemories').startSpan('mergeWithRemote');
        const seen = new Set();
        const canonicalId = (m) => `${m.tool}:${m.file}:${m.ruleId ?? ''}:${m.id}`;
