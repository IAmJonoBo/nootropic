...
async function getStatusFromMarkdown(file) {
    ...
    if (lines.includes('status: implemented') || lines.includes('status: complete'))
        return 'implemented';
    if (lines.includes('status: planned') || lines.includes('planned:'))
        return 'planned';
    if (lines.includes('status: in progress') || lines.includes('in progress'))
        return 'in progress';
...
async function main() {
    ...
    const allMdFiles = await getAllMarkdownFiles(capRoot);
    const implemented = [];
    const planned = [];
    const inProgress = [];
    const missingStatus = [];
    ...
    const getMarkdownSlugs = async (dir) => {
