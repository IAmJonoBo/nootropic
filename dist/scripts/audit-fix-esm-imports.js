...
function walk(dir, cb) {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory())
            walk(full, cb);
        else if (entry.isFile() && full.endsWith(EXT) && !IGNORE_FILES.some(r => r.test(full)))
            cb(full);
    }
}
function fixImports(content, addTsIgnore = false) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        line = line.replace(/(import|export)([^'"`]*?)(['"])((?:[.]{1,2}|\/)[^'"`]+?)(\.ts)?(['"])/g, (m, imp, pre, q, pth, _ts, post) => {
            if (imp.includes('type'))
                return m; // skip import type
            if (!pth.endsWith('.js'))
                return `${imp}${pre}${q}${pth}.js${post}`;
            return m;
        });
