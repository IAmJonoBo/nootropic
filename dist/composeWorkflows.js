    ...
    for (const step of sortedSteps) {
        ...
        switch (step.type ?? 'task') {
            case 'task': {
                ...
                const agent = registry.find((r) => typeof r === 'object' && r !== null && 'name' in r && r.name === step.agent);
                if (!agent)
                    throw new Error(`Agent/tool '${step.agent}' not found in registry`);
                // Policy/mutation hook: rationale event
                if (options?.rationaleHandler)
                    await options.rationaleHandler(step, `Executing task step ${step.id}`);
            ...
            case 'approval': {
                ...
                if (options?.rationaleHandler)
                    await options.rationaleHandler(step, `Approval required for step ${step.id}`);
            ...
            case 'parallel': {
                ...
                if (Array.isArray(children)) {
                    await Promise.all(children.map(async (childId) => {
                        const childStep = parsed.steps.find((s) => typeof s === 'object' && s !== null && 'id' in s && s.id === childId);
                        if (!childStep)
                            throw new Error(`Parallel child step '${childId}' not found`);
                ...
                if (options?.rationaleHandler)
                    await options.rationaleHandler(step, `Parallel group completed for step ${step.id}`);
            ...
            case 'loop': {
                ...
                if (options?.rationaleHandler)
                    await options.rationaleHandler(step, `Loop executed for step ${step.id}`);
        ...
        if (options?.postStep)
            await options.postStep(step, results[step.id]);
    }
    return results;
}
/**
