...
async function main() {
    ...
    let result = { status: 'no_action', message: '', event: undefined };
    try {
        ...
        async function postComment(issue_number, body) {
            await octokit.issues.createComment({ owner: owner, repo: repo, issue_number, body });
            console.log(`[comment] #${issue_number}: ${body}`);
        }
        // Helper: add label(s)
        async function addLabels(issue_number, labels) {
            await octokit.issues.addLabels({ owner: owner, repo: repo, issue_number, labels });
            console.log(`[label:add] #${issue_number}: ${labels.join(', ')}`);
        }
        // Helper: remove label(s)
        async function removeLabel(issue_number, label) {
            try {
                await octokit.issues.removeLabel({ owner: owner, repo: repo, issue_number, name: label });
