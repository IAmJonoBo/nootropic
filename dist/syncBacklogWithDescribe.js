...
async function main() {
    ...
    let archive = [];
    try {
        archive = await fsp.readFile(archivePath, 'utf-8').then(JSON.parse);
    }
    catch { }
    const registry = await fsp.readFile(describePath, 'utf-8').then(JSON.parse);
    const implemented = registry.filter((d) => {
        if (typeof d !== 'object' || d === null)
            return false;
        const obj = d;
        return obj['status'] === 'implemented' || obj['status'] === 'complete';
    });
    const implementedKeys = new Set(implemented.flatMap((d) => [
        normalize(d['name'] ?? ''),
        normalize(d['id'] ?? '')
    ]));
    ...
    for (const item of backlogItems) {
        const itemKeys = [
            normalize(item['id'] ?? ''),
            normalize((item['description'] ?? '').split(':')[0] ?? '')
        ];
    ...
    const backlogKeys = new Set(backlogItems.flatMap((i) => [
        normalize(i['id'] ?? ''),
        normalize((i['description'] ?? '').split(':')[0] ?? '')
    ]));
    const missing = implemented.filter((d) => {
        const keys = [
            normalize(d['name'] ?? ''),
            normalize(d['id'] ?? '')
        ];
    ...
    if (missing.length) {
        console.warn('WARNING: Implemented features missing from backlog:', missing.map((d) => {
            if (typeof d !== 'object' || d === null)
                return '';
            const obj = d;
