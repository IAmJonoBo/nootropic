...
> **Note:** The canonical sources for all documentation, backlog, and LLM/agent introspection are [`docs/docManifest.json`](./docs/docManifest.json) and [`.nootropic-cache/describe-registry.json`](./nootropic-cache/describe-registry.json). All code/doc sync, describe registry, and CI enforcement follow [2025 TypeScript monorepo best practices](https://medium.com/@nikhithsomasani/best-practices-for-using-typescript-in-2025-a-guide-for-experienced-developers-4fca1cfdf052). See CONTRIBUTING.md for details.
...
- **Doc Manifest:** [docs/docManifest.json](./docs/docManifest.json) (machine-readable, always up to date)
- **Backlog:** [agentBacklog.json](./agentBacklog.json) (annotated, canonical)
...
- The doc manifest ([docs/docManifest.json](./docs/docManifest.json)) now includes all capabilities, epics, and stories for LLM/agent workflows and automation.
...
- All automation scripts and navigation are updated for the new structure. See [generateDocManifest.ts](./generateDocManifest.ts) and [generateBacklogSummary.ts](./generateBacklogSummary.ts).
...
- **References:**
  - [`workflowSchema.ts`](./workflowSchema.ts) (schema)
  - [`composeWorkflows.ts`](./composeWorkflows.ts) (loader/runner)
  - [`orchestration/examples/simple-workflow.yaml`](./orchestration/examples/simple-workflow.yaml) (example)
  - [`docs/orchestration.md`](./docs/orchestration.md) (architecture, best practices)
