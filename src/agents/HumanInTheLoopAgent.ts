...
export class HumanInTheLoopAgent extends BaseAgent {
  ...
  override async runTask(task: unknown): Promise<AgentResult> {
    const safeTask = (typeof task === 'object' && task !== null) ? task as Record<string, unknown> : { value: task };
    const logs: string[] = [];
    ...
    return { output: { echo: safeTask }, success: true, logs: [...logs, ...events.map(e => `[event] ${e.type}: ${JSON.stringify((e && typeof e === 'object' && 'payload' in e ? (e as { payload?: unknown }).payload : {}) ?? {})}`)] };
  }
