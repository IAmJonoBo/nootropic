...
function isBacklogTodo(item: unknown): item is BacklogTodo {
  return (
    typeof item === 'object' &&
    item !== null &&
    'id' in item &&
    typeof (item as { id: unknown }).id === 'string' &&
    ((item as { id: string }).id.startsWith('todo:') || (item as { id: string }).id.startsWith('epic-'))
  );
}
...
async function main() {
  ...
  try {
    ...
    const todos: BacklogTodo[] = Array.isArray(backlogJson.backlog)
