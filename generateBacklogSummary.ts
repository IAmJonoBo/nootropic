...
function extractActionable(backlog: unknown[]): unknown[] {
  ...
  for (const item of backlog) {
    ...
    const entry: any = { id, description, status: s, priority, note, references, slug, docPath };
    if (Array.isArray(subtasks) && subtasks.length) {
      entry.subtasks = extractActionable(subtasks).map((sub: any) => ({ ...sub, epicSlug: slug, epicDocPath: docPath }));
    }
